...
Te_test = function(x,y,given=FALSE,P=NULL,Q=NULL,print=TRUE)
{
  ...
  n = length(y)
  if(m<n){
    pvalue = Te_test0(y,x,given,P,Q,print)
    pvalue <- c(pvalue[2],pvalue[1],pvalue[4],pvalue[3],-pvalue[5],pvalue[6],pvalue[7])
    names(pvalue)<-c("mean_x","mean_y","sd_x","sd_y","Te statistics","p_value","df")
  }else{
    pvalue = Te_test0(x,y,given,P,Q,print)
  }
  pvalue
}
...
Te_CI = function(x,y,alpha=0.05)
{
  m = length(x)
  n = length(y)
  if(m<n) pvalue = sort(-Te_CI0(y,x,alpha))
...
All_test<- function(x,y,given=FALSE,P=NULL,Q=NULL){
  ...
  names(result) <- c("Te","Welch","Z","t test")
  return(result)
}


sim_Te<-function(mu1,mu2,sigma1,sigma2,n1,n2,rpt=1e4){
  result<-matrix(0,rpt,4)
  n<-min(n1,n2)
  m<-max(n1,n2)
  P = Pmat(n)
  Q = Qmat(m,n)
  ...
  error_CI<-matrix(round(
    c(type_one_error,
      type_one_error -1.96 * sqrt(type_one_error*(1-type_one_error)/rpt),
      type_one_error +1.96 * sqrt(type_one_error*(1-type_one_error)/rpt)),4),4,3)
  colnames(error_CI)<-c("type_one_error","lower bound","upper bound")
  rownames(error_CI)<-c("Te","Welch","Z","t test")
